...
async def broadcast_tool_event(
    agent_id: str, tool_name: str, result: str, latency_ms: float
) -> None:
    try:
...
async def broadcast_team_node_event(
    team_id: str, node_id: str, status: str, output: str = ""
) -> None:
    try:
        mgr = get_monitor_manager()
        await mgr.broadcast(
...
def broadcast_tool_event_sync(
    ...
    try:
        ...
        asyncio.ensure_future(
            broadcast_tool_event(agent_id, tool_name, result, latency_ms)
        )
    except RuntimeError:
