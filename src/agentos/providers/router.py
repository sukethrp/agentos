...
def call_model(
    ...
    if provider == "openai":
        ...
        return call_llm(messages, tools, model=model, temperature=temperature, max_tokens=max_tokens, agent_name=agent_name)

    elif provider == "anthropic":
        from agentos.providers.anthropic_provider import call_anthropic
        return call_anthropic(messages, tools, model=model, temperature=temperature, max_tokens=max_tokens, agent_name=agent_name)

    elif provider == "ollama":
        from agentos.providers.ollama_provider import call_ollama
        actual_model = model.replace("ollama:", "")
        return call_ollama(messages, tools, model=actual_model, temperature=temperature, max_tokens=max_tokens, agent_name=agent_name)
...
def list_providers() -> dict:
    ...
    return {
        ...
        "ollama": ["ollama:<any-model>", "ollama:llama3.1", "ollama:mistral", "ollama:gemma2"],
    }