...
def _chunk_fixed(text: str, chunk_size: int, chunk_overlap: int, metadata: dict) -> list[Chunk]:
    chunks = []
    start = 0
    i = 0
    while start < len(text):
        ...
        if len(chunk_text) >= 50:
            chunks.append(Chunk(text=chunk_text, metadata={**metadata, "chunk_index": i}, index=i))
            i += 1
        start += chunk_size - chunk_overlap
    return chunks


def _chunk_sentence(text: str, chunk_size: int, chunk_overlap: int, metadata: dict) -> list[Chunk]:
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    text = re.sub(r"\n{3,}", "\n\n", text).strip()
    if not text:
        return []
...
def _chunk_semantic(text: str, chunk_size: int, chunk_overlap: int, metadata: dict) -> list[Chunk]:
...
class IngestionPipeline:
    ...
    def ingest_path(self, path: str) -> int:
        ...
        total = 0
        for f in p.rglob("*"):
            if f.is_file() and f.suffix.lower() in (".txt", ".md", ".rst", ".csv", ".log", ".pdf"):
                total += self._ingest_file(f)
        return total

    def _ingest_file(self, path: Path) -> int:
        text = self._read_file(path)
        meta = {"source": str(path), "filename": path.name, "filetype": path.suffix.lower()}
