...
class ChromaStore(BaseVectorStore):
    ...
    def add_batch(
        ...
        self._collection.add(
            ids=ids, embeddings=embeddings, documents=texts, metadatas=safe_meta
        )
        return list(
            range(self._collection.count() - len(texts), self._collection.count())
        )
    ...
    def search(
        ...
        for i, (doc, meta, did) in enumerate(zip(docs, metas, ids)):
            ...
            if sim >= threshold:
                results.append(
                    SearchResult(
                        text=doc, score=sim, metadata=meta or {}, doc_id=did, index=i
                    )
                )
...
class PineconeStore(BaseVectorStore):
    def __init__(
        ...
        pc = Pinecone(
            ...
            environment=environment or os.environ.get("PINECONE_ENV", "us-east1-gcp"),
        )
    ...
    def add_batch(
        ...
        doc_ids = doc_ids or [f"doc_{i}" for i in range(len(texts))]
        vectors = []
        for i, (emb, text, meta, did) in enumerate(
            zip(embeddings, texts, metadatas, doc_ids)
        ):
...
class PgVectorStore(BaseVectorStore):
    def __init__(
        self, collection_name: str = "default", connection_string: str | None = None
    ):
        import psycopg
    ...
    def add_batch(
        ...
        indices = []
        for i, (text, emb, meta, did) in enumerate(
            zip(texts, embeddings, metadatas, doc_ids)
        ):
            cur.execute(
